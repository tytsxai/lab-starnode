...
- `apps/web/components/EditorPanel.tsx`：编辑器面板、标签输入、快捷提交与笔记编辑模式。
- `apps/web/components/LinkPanel.tsx`：关联解释列表与关联筛选开关。
- `apps/web/components/UniversePanel.tsx`：3D 宇宙视图、关联解释面板、笔记搜索/排序/标签过滤、冰封视图切换与批量操作入口。
...
```txt
apps/web
  -> @starnode/core      (读取领域模型与计算规则)
  -> @starnode/renderer  (展示 3D 视图)
  -> @starnode/storage   (本地持久化)
  -> zustand             (前端状态管理)

@starnode/renderer
  -> @starnode/core      (消费 PlanetViewModel)
...
3. `storage` 只负责读写，不推导业务状态。
4. `apps/web` 负责组装与交互流程。

## 关键架构决策
1. 先做 **本地优先 + 单人宇宙**，保证闭环体验。
2. 先做 **规则关联**，AI 关联后置到 P1/P2。
